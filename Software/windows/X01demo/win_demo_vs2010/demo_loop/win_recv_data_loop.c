...
unsigned int CALLBACK recv_callback(PVOID context)
{
	...
	int save_file_flag, dev_open_flag, reg_int_flag, dev_enab_flag, monitor_flag;
	...
	do {
		...
		while (g_stop == 0)
		{
			buf = (FCUCHAR *)&va_addr;
			nLen = sizeof(int);
			ret = FCM_RecvData(g_dev, (void **)&buf, &nLen);
			if (ret)
			{
				printf("recv data failed ret=%d err=%d\n", ret, GetLastError());
				break;
			}
			if (nLen == 0) continue;
			if (g_save_file == 1)
			{
				w_len = fwrite(buf, nLen, 1, g_fp);
				if (w_len != 1)
				{
			...
			parse_buf(buf, nLen);
			/*释放缓冲区*/
			ret = FCM_ReleaseBuf(g_dev, buf);
			if (ret)
			{
				printf("release buf failed ret=%d err=%d\n", ret, GetLastError());
				break;
			}
		}
	} while (FALSE);
...
void print_statistics()
{
	...
	s = &g_demo_stat;
	printf("statistics[bytes=%I64d frame_num=%I64d cfgid_err=%I64d monitorid_err=%I64d crc_err=%I64d undersize=%I64d oversize=%I64d no_eof_err=%I64d disp_err=%I64d coding_err=%I64d]\n",
