...
VOID Test_FCM_DevEnable()
{
    ...
    ret = FCM_DevEnable(gDev, fcid, credit_num);
    if (ret)
    {
...
VOID parse_frame(char *buf, int length, PFCM_DEMO_STATISTICS v)
{
	...
	// 小端处理
	asmid = 0;
	asmid = (unsigned char)buf[32];
	asmid = (asmid << 8) | (unsigned char)buf[32 - 3];
	asmid = (asmid << 8) | (unsigned char)buf[32 - 2];
	asmid = (asmid << 8) | (unsigned char)buf[32 - 1];
	assert(asmid > 0 && asmid < 256);
	v->asm_counter[asmid]++;
#endif
}
